...
const __builtinRequire = function(id) {
  ...
  if (bare === 'crypto') return {
    ...
    createHash(alg) {
      // Minimal sync SHA-256 (and SHA-1) for NAPI worker — cannot import main polyfill
      const name = String(alg || 'sha256').toLowerCase().replace(/[^a-z0-9]/g, '');
      const parts = [];
      ...
      return {
        update(chunk) {
          const u = typeof chunk === 'string' ? new TextEncoder().encode(chunk)
        ...
        digest(enc) {
          ...
          const hashed = (name === 'sha1') ? sha1(data) : sha256(data);
          if (enc === 'hex') {
            let s = ''; for (let i = 0; i < hashed.length; i++) s += hashed[i].toString(16).padStart(2, '0');
            return s;
          }
          return hashed;
        },
