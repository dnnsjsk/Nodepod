...
export type InspectWaitUntil = "domcontentloaded" | "load" | "networkidle" | "settled" | {
    selector: string;
    timeout?: number;
};
...
export interface InspectInteractiveNode {
    ref: string;
    role: string;
    name: string;
    tag: string;
...
export interface InspectNetworkFailure {
    method: string;
    url: string;
    status: number;
    response?: string;
...
export declare class PreviewInspector {
    ...
    interactives(target: InspectTarget): Promise<InspectResult<InspectInteractiveNode[]>>;
    act(target: InspectTarget & InspectAction): Promise<InspectResult<InspectActionResult>>;
    networkFailures(target: InspectTarget & {
        since?: number;
    }): Promise<InspectResult<InspectNetworkFailure[]>>;
